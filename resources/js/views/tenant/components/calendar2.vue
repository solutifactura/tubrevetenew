<template>
    <div class="box ">
        <div class="box-body no-padding">
            ...
            <div class="el-form-item  col-xs-12">
                <div class="el-form-item__content">
                    <el-select v-model="vendedor" name="vendedor" clearable placeholder="Usuario">
                        <el-option v-for="item in vendedores" :key="item.id" :label="item.name.toUpperCase()" :value="item.id"></el-option>
                    </el-select>
                </div>
            </div>
...
<script>
    export default {
        ...
        created() {
            this.vendedor = (this.td != '') ? this.vendedores.find(row => row.id == this.td).id : null; 
