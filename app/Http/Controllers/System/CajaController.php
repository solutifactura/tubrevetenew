...
{   
    ...
    {   
        ...
        return view('system.caja.index', compact('clients'));
    }

    public function create()
    {
        return view('system.caja.form');
    }
    ...
    public function search(Request $request)
    {
        ...
        if ($request->has('d') && $request->has('a') && ($request->d != null && $request->a != null)) {
            ...
            foreach ($reports as &$row) {
                $tenancy = app(Environment::class);
                $tenancy->tenant($row->hostname->website);
                $row->count_doc = DB::connection('tenant')->table('documents')->whereBetween('date_of_issue', [$d, $a])->count();
                $row->sum_tg = DB::connection('tenant')->table('documents')->whereBetween('date_of_issue', [$d, $a])->sum('total_taxes');
                $row->sum_igv = DB::connection('tenant')->table('documents')->whereBetween('date_of_issue', [$d, $a])->sum('total_value');
                $row->sum_total = DB::connection('tenant')->table('documents')->whereBetween('date_of_issue', [$d, $a])->sum('total');
            }
           
        }


        return view("system.caja.index", compact("reports", "a", "d", "client","clients"));
    ...
    public function pdf(Request $request) {
        ...
        $pdf = PDF::loadView('system.caja.report_pdf', compact("reports", "a", "d"));
    ...
    public function excel(Request $request) {
        ...
        return (new DocumentExportSystem)
                ->records($records)
                ->d($d)
                ->a($a)
