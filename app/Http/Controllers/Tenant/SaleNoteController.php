...
class SaleNoteController extends Controller
{
    ...
    public function searchAlumnos(Request $request)
    {
        ...
        $alumnos = Person::where('number','like', "%{$request->input}%")
                            ->orWhere('name','like', "%{$request->input}%")
                            ->whereType('alumnos')->orderBy('name')                           
                            ->get()->transform(function($row) {
                                return [
                                    'id' => $row->id,
                                    'description' => $row->number.' - '.$row->name,
    ...
    public function searchVendedores(Request $request)
    {
        ...
        $vendedores = Person::where('number','like', "%{$request->input}%")
                            ->orWhere('name','like', "%{$request->input}%")
                            ->whereType('vendedores')->orderBy('name')                           
                            ->get()->transform(function($row) {
                                return [
                                    'id' => $row->id,
                                    'description' => $row->number.' - '.$row->name,
    ...
    public function tables()
    {
        ...
        $alumnos =  $this->table('alumnos');
        $vendedores =  $this->table('vendedores');
        ...
        return compact('customers', 'alumnos', 'vendedores', 'establishments','currency_types', 'discount_types',
    ...
    public function mergeData($inputs)
    {
        ...
        $values = [
            ...
            'alumno' => PersonInput::set($inputs['alumno_id']),
            'vendedor' => PersonInput::set($inputs['vendedor_id']),
...
//    }
    ...
    public function createPdf($sale_note = null, $format_pdf = null, $filename = null) {
        ...
        if (($format_pdf === 'ticket') OR ($format_pdf === 'ticket_58')) {
            ...
            $alumno_name     = strlen($this->document->alumno->name) > '25' ? '10' : '0';
    ...
    public function table($table)
    {
        switch ($table) {
            case 'customers':
                ...
                $alumnos = Person::whereType('alumnos')->orderBy('name')->take(20)->get()->transform(function($row) {
                    return [
                        'id' => $row->id,
                        'description' => $row->number.' - '.$row->name,
                        'name' => $row->name,
                ...
                $vendedores = Person::whereType('vendedores')->orderBy('name')->take(20)->get()->transform(function($row) {
                    return [
                        'id' => $row->id,
                        'description' => $row->number.' - '.$row->name,
                        'name' => $row->name,
...
//                return $items;
    ...
    public function searchAlumnoById($id)
    {        
   
        $alumnos = Person::whereType('alumnos')
                    ->where('id',$id) 
                    ->get()->transform(function($row) {
                        return [
                            'id' => $row->id,
    ...
    public function searchVendedorById($id)
    {        
   
        $vendedores = Person::whereType('vendedores')
                    ->where('id',$id) 
                    ->get()->transform(function($row) {
                        return [
                            'id' => $row->id,
